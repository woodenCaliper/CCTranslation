...
├═══━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━═──────═┤ ← 分割バー（ドラッグ可能）。縮小時は ▶ 原文のすぐ下の行に表示する
│ 翻訳結果 (ja)                              │ ← 翻訳結果タイトル行
│ ┌───────────────────────────────────────┐ │ ← 翻訳後テキストエリア（ここから）
│ │ こんにちは、お元気ですか？              │ │   （翻訳結果表示）
│ │ これはサンプルテキストです。             │ │
...
- **原文テキストエリア**:
  - **自動調整**: 文章量に応じて高さを自動調整
  - **最小高さ**: 1行
  - **最大高さ**: 6行（長い文章の場合はスクロール表示）
  - **スクロール**: 縦スクロールバーで長文対応
  - **折り返し**: テキストの自動折り返し（改行なしテキスト対応）
  - **フォント**: Google翻訳と同様のフォント（Noto Sans JP / Roboto）
  - **縮小状態**: 縮小状態時は非表示となり、その表示エリアも開放する
- **分割バー**: 翻訳前後エリアの境界を上下ドラッグして高さ調整
  - **最小サイズ制限**: 分割バーをドラッグしても、原文エリアと翻訳結果エリアの最小高さは維持する
  - **ドラッグ制限**: どちらかのエリアが最小サイズに達した場合は、それ以上ドラッグできないよう制限する
- **翻訳結果テキストエリア**:
  - **自動調整**: 文章量に応じて高さを自動調整
  - **最小高さ**: 1行
  - **最大高さ**: 6行（長い文章の場合はスクロール表示）
...
**原文縮小要件**
- **縮小方法**: 
  - ▼ 原文 (en)をクリックすると、原文テキストエリアを非表示にします。原文テキストエリアの高さをゼロにして、その下にある項目の位置を非表示にした文だけ上に移動させる。
  - この時、原文テキストエリアは非表示になるので、最小高さの制限は無くなり、分割バーの位置を阻害しない。
